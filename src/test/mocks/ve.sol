...
contract ve is IERC721, IERC721Metadata {
    ...
    string public constant name = "veHERMES";
    string public constant symbol = "veHERMES";
    ...
    mapping(uint256 => uint256) internal idToVoteExpiry;
    ...
    /// @param _tokenId The identifier for an NFT.
    function isVoteExpired(uint256 _tokenId) external view returns (bool) {
        return _isVoteExpired(_tokenId);
    }

    /// @dev Returns true if NFT is unlocked, false if locked in vote.
    /// @param _tokenId The identifier for an NFT.
    function _isVoteExpired(uint256 _tokenId) internal view returns (bool) {
        return idToVoteExpiry[_tokenId] < block.timestamp;
    }

    /// @dev Returns remaining vote lock for an NFT.
    /// @param _tokenId The identifier for an NFT.
    function voteExpiry(uint256 _tokenId) external view returns (uint256) {
    ...
    /// @param _tokenId The identifier for an NFT.
    function lockVote(uint256 _tokenId) external {
    ...
    /// @param _to Address to deposit
    function _create_lock_minter(
        uint256 _value,
        uint256 _lock_duration,
        address _to
    ) internal returns (uint256) {
        require(block.timestamp < 1647043200, "Only allowed before first distribution");
        uint256 unlock_time = ((block.timestamp + _lock_duration) / WEEK) * WEEK - WEEK; // Locktime is rounded down to last week
    ...
    function create_lock_for_minter(
        ...
        return _create_lock_minter(_value, _lock_duration, _to);
    }
